---
name: juliopy
description: Scaffold a production-grade Python backend service — Docker, Postgres, Django or FastAPI, uv, ruff, mypy, pytest, CI, pre-commit, typed config, health checks, dev-ready hot reload, Makefile, README — always on current latest-stable versions. Use when starting a new Python backend/API project, containerizing a Python service, or setting up a professional Django/FastAPI + Postgres stack.
---
...
The Dockerfile has more than one final stage (`dev`, `production` — see step 5), so `docker-compose.yml`'s `build:` must set `target: production` explicitly; without it, `docker build` silently builds whichever stage is written last.

### 5. Make it dev-ready: hot reload, docker-compose.override.yml, Makefile

A scaffold nobody can iterate on quickly isn't actually production-grade — read `reference/dev-environment.md` and add `docker-compose.override.yml` (Compose merges it into `docker-compose.yml` automatically for a plain `docker compose up`) and a `Makefile`. The override points the build at the Dockerfile's `dev` stage, bind-mounts the source for live editing, and runs the framework's own autoreloading dev server (`uvicorn --reload` / Django's `runserver`) instead of gunicorn. Get the venv-shadowing detail right (documented in the reference file) or the dev container boots with a broken virtualenv.

### 6. Configure tooling

Read `reference/tooling.md` and add its `[tool.ruff]`, `[tool.mypy]`, and `[tool.pytest.ini_options]` blocks to `pyproject.toml`.

### 7. Wire CI and pre-commit
...
Read `reference/readme-template.md` and write a real `README.md`: prerequisites, quick start (`cp .env.example .env` then `docker compose up --build`), the `Makefile` command table from step 5, how dev vs. production builds differ, the health-check endpoints, and a one-paragraph project structure — filled in with this project's actual name, framework, and choices, not left as template placeholders. This is the thing a new contributor reads first; if it says something the scaffold doesn't actually do, that's a bug in the scaffold.

### 10. Verify — every item, not just the first

- `docker compose up --build` starts cleanly and the app connects to Postgres
- `/health` and `/ready` both return 200
- Hot reload actually works: with the stack running via plain `docker compose up`, edit a response string in a handler/view on the host and confirm the change shows up on the next request *without* rebuilding or restarting anything
- `docker compose -f docker-compose.yml up --build` (override excluded) builds and runs the `production` target cleanly on its own
- Every command in the README's command table (`make up`, `make test`, `make lint`, ...) actually does what the README says
